...
class DataProcessor(ABC):
    ...
    def replace_nan_value(self,df)-> pandas.DataFrame:
...
class CsvDataProcessor(DataProcessor):
    ...
    #    Сортируем данные по значениям колонки "index" и сохраняем результат в атрибуте result
    def run(self):
        keep_col = ['rating','name','release_date','director']
        new_f = self._dataset[keep_col]
        new_f.to_csv("newFile.csv", index=False)
    #    self.result = self.replace_nan_value(self.result)
        self.result = self.sort_data_by_col(new_f, "rating", True)
...
#class TxtDataProcessor(DataProcessor):
  ...
  #  def run(self):
     #   self.result = self.sort_data_by_col(self._dataset, "index", True)
